...
struct SingleProdSingleConsQueue {
  ...
  SingleProdSingleConsQueue () : head(0), tail(0) {
    cout << boolalpha << "Lock free:" <<  this->isLockFree() << endl;
  } 
...
struct Engine {
  Engine(Notifier& notifier) : books(), notify(notifier), isShutdown(false) {
    books['A'] = {};
    books['B'] = {};
    books['C'] = {};
  }
...
struct TradingTool {
  TradingTool(uint16_t identifier) : id(identifier), isShutdown(false) {}
